...
if (!seal.ext.find('b64')) {
    ...
    cmdBtoa.name = 'btoa';
    cmdBtoa.help = 'base64 加密';
    cmdBtoa.solve = (ctx, msg, cmdArgs) => {
    ...
    // 注册命令
    ext.cmdMap['btoa'] = cmdBtoa;
    //第二个指令
    const cmdAtob = seal.ext.newCmdItemInfo();
    cmdAtob.name = 'atob';
    cmdAtob.help = 'base64 解密';
    cmdAtob.solve = (ctx, msg, cmdArgs) => {
        let val = cmdArgs.getArgN(1)
        switch (val) {
            case 'help': {
                const ret = seal.ext.newCmdExecuteResult(true);
